 ...
 */
export async function updateLoaStatus(loaId, update) {
  try {
 ...
 */
export async function getLoaById(loaId) {
  try {
    // Read the current LOAs
    const loasData = fs.readFileSync(LOA_FILE_PATH, 'utf8');
 ...
 */
export async function getLoaMessageId(loaId) {
  try {
 ...
 */
export async function storeLoaMessageId(loaId, messageId) {
  try {
    // The path to the LOA message mapping file
    const mappingPath = path.join(__dirname, '../data/loa-message-map.json');
